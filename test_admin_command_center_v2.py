...
def load_runtime(tmp_path):
    ...
    spec = importlib.util.spec_from_file_location("admin_runtime_test", module_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    module.init_extension_schema(app)
    return module, app


def test_parse_update_course_is_structured(tmp_path):
    runtime, _ = load_runtime(tmp_path)
...
def test_create_and_update_course_records_history(tmp_path):
    runtime, app = load_runtime(tmp_path)
    ...
    course = conn.execute(
        "SELECT price FROM academy_courses WHERE name='اختبار'"
    ).fetchone()
    history_count = conn.execute(
        "SELECT COUNT(*) FROM academy_change_history WHERE entity_type='course'"
    ).fetchone()[0]
    conn.close()
...
def test_offer_is_date_bounded(tmp_path):
    ...
    conn = sqlite3.connect(app.DB_PATH)
    row = conn.execute(
        "SELECT title, starts_at, ends_at FROM academy_offers"
    ).fetchone()
    conn.close()
