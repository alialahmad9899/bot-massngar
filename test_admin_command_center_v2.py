...
def load_runtime(tmp_path):
    ...
    app.get_db_connection = lambda: sqlite3.connect(app.DB_PATH, timeout=10.0, check_same_thread=False)

    conn = sqlite3.connect(app.DB_PATH)
    conn.execute("""CREATE TABLE academy_courses(
        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, lessons INTEGER,
        duration_text TEXT, days_per_week INTEGER, price INTEGER, first_payment INTEGER,
        start_date TEXT, topics TEXT, active INTEGER NOT NULL DEFAULT 1,
        created_at TEXT, updated_at TEXT)""")
    conn.execute("""CREATE TABLE course_batches(
        id INTEGER PRIMARY KEY AUTOINCREMENT, course_id INTEGER NOT NULL, start_date TEXT NOT NULL,
        schedule_text TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT)""")
    conn.execute("""CREATE TABLE academy_info(
        info_key TEXT PRIMARY KEY, info_value TEXT NOT NULL, updated_at TEXT)""")
...
def test_parse_update_course_is_structured(tmp_path):
    ...
    parsed = runtime.parse_admin_command("غيّر سعر دورة ميك أب احترافي إلى 850000 وغيّر الدفعة الأولى إلى 250000")
...
def test_offer_is_date_bounded(tmp_path):
    runtime, _, app = load_runtime(tmp_path)
    result = runtime.execute_structured(app, "admin1", "أضف عرض مكياج: خصم 100000، من 2026-09-01 إلى 2026-09-10")
    assert result["ok"] is True
    conn = sqlite3.connect(app.DB_PATH)
