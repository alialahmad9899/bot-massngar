...
def build_dynamic_academy_knowledge():
    ...
    for c in courses:
        lines.append(
            f"- {c['name']}: {c['lessons'] or '?'} درس؛ مدة الدرس {c['duration_text'] or 'غير محددة'}؛ {c['days_per_week'] or '?'} أيام بالأسبوع؛ السعر {c['price'] if c['price'] is not None else 'غير محدد'} (دورات جماعية)؛ الدفعة الأولى {c['first_payment'] if c['first_payment'] is not None else 'غير محددة'}؛ البداية {c['start_date'] or 'غير محددة'}؛ المحاور {c['topics'] or 'غير محددة'}"
        )
    if batches:
...
شخصيتك: مهني، مؤدب، محترم، لهجة سورية عامية راقية ومحترمة، ونبرة مبيعات استشارية وتفاعلية عملية.
✨ استخدام السمايلات: استخدم الإيموجيات الهادئة واللطيفة بشكل متزن وجميل يعطي دفئاً ولطافة للمحادثة (مثل: 😊, 🌸, 📍, 📌, ✨, 💳)، ودون مبالغة أو إفراط حشو.
...
⛔ حظر تام ومطلق لكلمات التدليل والميانة والألقاب العاطفية: يُمنع منعاً باتاً حزماً وقطعياً استخدام كلمات مثل: (غاليتي، غالي، يا غالي، عيوني لك، يا عيوني، حبيبتي، حبيبي، تؤبريني، لعيونك، عيني، يا بعد قلبي، أو أي ألفاظ غزل وتدليل أُخرى).
⛔ أسلوب الخطاب الرسمي والمحترم: استخدم عبارات الترحيب واللباقة الرسمية المهنية فقط بلهجة عامية راقية (مثل: "أهلاً بكِ"، "أهلاً بك"، "تفضلي"، "تفضل"، "تكرمي"، "على الرحب والسعة"، "بكل سرور").
⛔ ممنوع: الفواصل الجندرية (حابب/حابة)، والعبارات الخشبية الرسمية القاتلة (دواعي سرورنا).
...
def generate_ai_reply(sender_id, user_message, intent=None, is_admin=False, message_id=None):
    try:
        ...
        return "أهلاً بك! 😊 أنا المساعد الذكي للأكاديمية الدولية للتدريب المهني. يمكنك التواصل مع إدارة الأكاديمية مباشرة على الرقم: 0932775583 📱"
